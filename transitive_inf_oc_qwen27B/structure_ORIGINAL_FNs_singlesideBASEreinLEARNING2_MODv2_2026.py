...
def single_play(fnoise2, fplen2, falen2, fmaxval2, fsides2, frein2, fpunish2, frecweights2, fsigweights2, fstate2, frandunif22, frandunif42, frandunif62, frandunif82, frandunif102, fpairs2, frewards2):
    ...
    left_reward = frewards2[stateidx][0]
    right_reward = frewards2[stateidx][1]
...
@numba.jit
def nstepsfntest(fnoiseN, fmaxvalN, fsidesN, frecweightsN, fsigweightsN, frandunif2N, frandunif4N, frandunif6N, frandunif8N, frandunif10N, fnaturestatesN, fnsteps, fpairsN, fpairsRewN, freinN, fpunishN):
    # Pass len(fpairsN) as both fplen and falen so the side-based branch doesn't matter.
    # The test phase always operates on whatever pair array it was handed, regardless of side semantics.
    fcumsumNtest = 0
    ...
    for idxN in range(0, fnsteps):
        ...
        pair_stats[stateidx, 1] += fcumsumaddtest
        
    return frecweightsN, fsigweightsN, fcumsumNtest, pair_stats


def play_sequence(n, rng, rein1, punish1, rein2, punish2, timesteps, nsteps, sides, pairs, testpairs, nonadjpairs, allpairs, pairs_reward, testpairs_reward, nonadjpairs_reward, allpairs_reward, plen, alen, terms, maxvalue, startstop, noise, annealing, runs, inertia, blocklength, nsteps_eval):
    ...
    for t in range(0, timesteps // nsteps):
        ...
        if ((t + 1) * nsteps) % blocklength == 0:
            ...
            # annealing with floor at 0
            cur_noise = max(0, cur_noise - annealing)
            if iterswitch == 0:
                rein = rein1
                punish = punish1
            else:
        ...
        recweights, sigweights, cumsucadd = nstepsfn(cur_noise, plen, alen, maxvalue, naturesides, recweights, sigweights, randunif2, randunif4, randunif6, randunif8, randunif10, naturestates, nsteps, allpairs, allpairs_reward, rein, punish)
        cumsuc += cumsucadd
     
    # test phase on held-out test pairs (read-only: pass weight copies, discard returned weights)
    naturestates, randunif2, randunif4, randunif6, randunif8, randunif10, naturesides = randoms(rng, nsteps_eval, maxvalue, 1, len(testpairs), sides)
    _, _, testcumsucadd, test_pair_stats = nstepsfntest(cur_noise, maxvalue, naturesides, recweights.copy(), sigweights.copy(), randunif2, randunif4, randunif6, randunif8, randunif10, naturestates, nsteps_eval, testpairs, testpairs_reward, rein, punish)
    
    # evaluation on all pairs (read-only: pass weight copies, discard returned weights)
    naturestates_eval, randunif2_eval, randunif4_eval, randunif6_eval, randunif8_eval, randunif10_eval, naturesides_eval = randoms(rng, nsteps_eval, maxvalue, 1, len(allpairs), sides)
    _, _, evalcumsucadd, eval_pair_stats = nstepsfntest(cur_noise, maxvalue, naturesides_eval, recweights.copy(), sigweights.copy(), randunif2_eval, randunif4_eval, randunif6_eval, randunif8_eval, randunif10_eval, naturestates_eval, nsteps_eval, allpairs, allpairs_reward, rein, punish)
    
    return sigweights, cumsuc, cumsucadd, testcumsucadd, recweights, test_pair_stats, evalcumsucadd, eval_pair_stats
