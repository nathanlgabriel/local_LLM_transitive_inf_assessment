...
def nstepsfntest(fnoiseN, fplenN, falenN, fmaxvalN, fsidesN, frecweightsN, fsigweightsN, frandunif2N, frandunif4N, frandunif6N, frandunif8N, frandunif10N, fnaturestatesN, fnsteps, fpairsN, frewardsN, freinN, fpunishN):
    fcumsumNtest = 0
    npairs = len(fpairsN)
    pair_stats = np.zeros((npairs, 2), dtype=numba.int64)
    for idxN in range(0, fnsteps):
        #randomly determin state of nature
        fstateN = fnaturestatesN[idxN]
        # perform a single play
        frecweightsNtest, fsigweightsNtest, fcumsumaddtest = single_play(fnoiseN, npairs, npairs, fmaxvalN, [0, 0], freinN, fpunishN, frecweightsN, fsigweightsN, fstateN, frandunif2N[idxN], frandunif4N[idxN], frandunif6N[idxN], frandunif8N[idxN], frandunif10N[idxN], fpairsN, frewardsN)
        fcumsumNtest += fcumsumaddtest
        stateidx = fstateN % npairs
...
def play_sequence(n, rng, rein1, punish1, rein2, punish2, timesteps, nsteps, sides, pairs, testpairs, nonadjpairs, allpairs, pairs_reward, testpairs_reward, nonadjpairs_reward, allpairs_reward, plen, alen, terms, maxvalue, startstop, noise, annealing, runs, inertia, blocklength):
    ...
    recweights, sigweights, testcumsucadd, test_pair_stats = nstepsfntest(noise, plen, alen, maxvalue, naturesides, recweights, sigweights, randunif2, randunif4, randunif6, randunif8, randunif10, naturestates, nsteps, testpairs, testpairs_reward, rein, punish)
    
    return sigweights, cumsuc, cumsucadd, testcumsucadd, recweights, test_pair_stats
