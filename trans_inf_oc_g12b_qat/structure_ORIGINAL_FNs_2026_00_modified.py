...
def nstepsfn(fnoiseN, fplenN, falenN, fmaxvalN, fsidesN, frecweightsN, fsigweightsN, frandunif2N, frandunif4N, frandunif6N, frandunif8N, frandunif10N, fnaturestatesN, fnsteps, fpairsN, frewardsN, freinN, fpunishN):
    fcumsumN = 0
    pair_stats = np.zeros((fpairsN.shape[0], 2), dtype=np.int64)
    for idxN in range(0, fnsteps):
        ...
        # Track stats for test pairs or all pairs depending on the call
        # The nstepsfn call in play_sequence uses allpairs
        # The nstepsfntest call uses testpairs
        # We want to track stats for whatever fpairsN is passed
        stateidx = fstateN % len(fpairsN)
        pair_stats[stateidx, 0] += 1
        pair_stats[stateidx, 1] += int(fcumsumadd)
        
    return frecweightsN, fsigweightsN, fcumsumN, pair_stats
...
def nstepsfntest(fnoiseN, fplenN, falenN, fmaxvalN, fsidesN, frecweightsN, fsigweightsN, frandunif2N, frandunif4N, frandunif6N, frandunif8N, frandunif10N, fnaturestatesN, fnsteps, fpairsN, frewardsN, freinN, fpunishN):
    ...
    for idxN in range(0, fnsteps):
        fstateN = fnaturestatesN[idxN]
        frecweightsNtest, fsigweightsNtest, fcumsumaddtest = single_play(fnoiseN, fplenN, falenN, fmaxvalN, [0, 0], freinN, fpunishN, frecweightsN, fsigweightsN, fstateN, frandunif2N[idxN], frandunif4N[idxN], frandunif6N[idxN], frandunif8N[idxN], frandunif10N[idxN], fpairsN, frewardsN)
